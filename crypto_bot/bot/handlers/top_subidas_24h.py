"""
Handler para /TopSubidas24H.
Top 10 criptos con mayor subida en las últimas 24 horas.
"""
...
async def top_subidas_24h(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Maneja el comando /TopSubidas24H.
    ...
    logger.info(f"Usuario {user_id} solicitó /TopSubidas24H")
    ...
    try:
        ...
        if not top_movers:
            ...
            metrics_manager.record_command("TopSubidas24H", user_id, False, response_time)
        ...
        # Registrar métrica
        response_time = (time.time() - start_time) * 1000
        metrics_manager.record_command("TopSubidas24H", user_id, True, response_time)

    except Exception as e:
        logger.error(f"Error en /TopSubidas24H: {e}")
        await update.message.reply_text(
