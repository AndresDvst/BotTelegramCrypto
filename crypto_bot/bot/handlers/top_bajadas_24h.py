"""
Handler para /TopBajadas24H.
Top 10 criptos con mayor bajada en las últimas 24 horas.
"""
...
async def top_bajadas_24h(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    """Maneja el comando /TopBajadas24H.
    ...
    logger.info(f"Usuario {user_id} solicitó /TopBajadas24H")
    ...
    try:
        # Obtener top movers 24h (bajadas)
        ...
        if not top_movers:
            ...
            metrics_manager.record_command("TopBajadas24H", user_id, False, response_time)
        ...
        # Registrar métrica
        response_time = (time.time() - start_time) * 1000
        metrics_manager.record_command("TopBajadas24H", user_id, True, response_time)

    except Exception as e:
        logger.error(f"Error en /TopBajadas24H: {e}")
        await update.message.reply_text(
